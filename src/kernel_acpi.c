        ...
        while(((uintptr_t)madt_entry - (uintptr_t)madt) < madt->header.length) 
        {
          ...
          switch(madt_entry->entry_type){
            case PROCESSOR_LOCAL_APIC: {
              ...
              klog_info("[acpi] lapic: processor_id: %u apic_id: %u is %s", local_apic->processor_id, local_apic->apic_id, is_apic_enabled ? "enabled" : "disabled");
              //TODO(Torin) This is garbage if the apic has failed!
              sys->processor_count++;
            } break;
